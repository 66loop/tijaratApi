...
exports.createOrder = async function (req, res) {
  ...
  for (let index = 0; index < req.body.cart.length; index++) {
    ...
    const orderIs = {
      ...
      paymentDone: element.paymentEvidence
        ? element.paymentEvidence.trxId !== undefined ||
          element.paymentEvidence.trxId !== "" ||
          element.paymentEvidence.screenshot !== "" ||
          element.paymentEvidence.screenshot !== undefined
        : false,
  ...
  Order.find({ "orders.seller": req.params.sellerId })
    ...
    .then((response) => {
      ...
      for (let index = 0; index < response.length; index++) {
        ...
        let differentSellers = element.orders
          .map((x) => x.product.seller)
          .filter((value, index, self) => self.indexOf(value) === index);

        if (differentSellers.length > 1) {
          let ordersSpecificToASeller = element.orders.filter(
            (item) => item.product.seller.toString() === req.params.sellerId
          );
          ordersShouldBe.push({
            _id: element._id,
            user: element.user,
            masterOrderNumber: element.masterOrderNumber,
            createdAt: element.createdAt,
            updatedAt: element.updatedAt,
            orders: ordersSpecificToASeller,
          });
        } else {
  ...
  Order.find({ _id: req.params.orderId })
    ...
    .then((response) => {
      ...
      for (let index = 0; index < response.length; index++) {
        const element = response[index];
        let differentSellers = element.orders
          .map((x) => x.product.serllerId)
          .filter((value, index, self) => self.indexOf(value) === index);

        if (differentSellers.length > 1) {
          let ordersSpecificToASeller = element.orders.filter(
            (item) => item.product.serllerId.toString() === req.params.sellerId
          );
          ordersShouldBe.push({
            _id: element._id,
            user: element.user,
...
exports.updateOrder = async function (req, res) {
  Order.updateOne(
    { _id: req.params.orderId, "orders.seller": req.query.seller },
    { "orders.$.orderStatus": req.query.status }
  )
    .then((response) => {
      Order.findOne({ _id: req.params.orderId }).then((res1) => {
        let differentOrderStatus = res1.orders
          .map((x) => x.orderStatus)
          .filter((value, index, self) => self.indexOf(value) === index);

        if (
          differentOrderStatus.length > 0 &&
          differentOrderStatus.length < 2
        ) {
          Order.updateOne(
            { _id: req.params.orderId },
            { overAllOrderStatus: differentOrderStatus[0] }
          ).then((response) => {
            res.status(200).json({
...
exports.getOrderByUser = async function (req, res) {
  try {
    let incompleteOrders = await Order.find({
      $and: [
        { buyer: req.params.userId },
        { overAllOrderStatus: { $ne: "Delivered" } },
      ],
    })
      .populate("user orders.product orders.seller orders.buyer")
      .sort({ createdAt: -1 });
    let completeOrders = await Order.find({
      $and: [{ buyer: req.params.userId }, { overAllOrderStatus: "Delivered" }],
    })
      .populate("user orders.product orders.seller orders.buyer")
      .sort({ createdAt: -1 });
    res.status(200).json({
...
exports.sendEmailForReview = async function () {
  ...
  let orders = await Order.find({
    $and: [
      { updatedAt: { $lt: todayDate } },
      { updatedAt: { $gt: dateDayBeforeYesterday } },
      { overAllOrderStatus: "Shipped" },
    ],
  }).populate("user");

  let body =
    '<p style="font-size:18px;">Your order has been delivered yesterday, please review the products in order by clicking the link below<br></br></p>' +
  ...
  if (orders.length > 0) {
    for (let index = 0; index < orders.length; index++) {
      try {
        ...
        body = body.replace(
          "{link}",
          constants.constants.baseUrl + "?reviewKey=" + reviewKey
        );
        element.user = { email: "usamadanish22@gmail.com" };
        await emailSending.sendEMessage(
          "Please review product you received",
          body,
